...
jlc-cli prefers `JLC_OBSCURA_EXECUTABLE`, `JLC_CHROME_EXECUTABLE`, `JLC_CHROME_HEADLESS`, and `JLC_CHROME_NO_SANDBOX`. Each falls back to its previous `FABRELAY_` equivalent when the new variable is unset. The old `fabrelay/obscura` cache layouts are still searched after the new cache layouts. Explicit executable configuration keeps precedence.

When neither `--home` nor a home environment variable is specified, `~/.jlc-cli` is used for new installations. If only `~/.fabrelay` exists, it is reused in place. The old private broker endpoints and instance header are retained so an already-running browser can be reconnected without replacing its page or login state. New broker processes use `--jlc-cdp-broker`; the old startup flag remains accepted for compatibility.
