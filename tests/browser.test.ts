...
const readControl = vi.hoisted(() => ({
  intercept: undefined as undefined | ((path: string) => Promise<void>),
}));
...
afterEach(async () => {
  readControl.intercept = undefined;
...
describe("CDP endpoint boundaries", () => {
  ...
  it("accepts and pins literal loopback", () => {
    expect(validateEndpoint("http://localhost:9222/")).toBe(
      "http://127.0.0.1:9222",
    );
    expect(validateEndpoint("ws://[::1]:9222/devtools/browser")).toBe(
      "ws://[::1]:9222/devtools/browser",
    );
  });
  it("drops all URL credentials and query fragments from diagnostics", () => {
    expect(
      sanitizeUrl(
        "https://alice:secret@jlc.com/orders?token=secret#access_token=x",
      ),
    ).toBe("https://jlc.com/orders");
    expect(sanitizeUrl("data:text/plain,SECRET")).toBe("data:[redacted]");
  });
  it("supports explicit paths and Windows candidates without shell execution", () => {
    ...
    expect(candidates[0]).toBe("C:/safe/obscura.exe");
    expect(candidates).toContain(join("D:/bin", "obscura.exe"));
  });
  it("reports missing Obscura without silently selecting Chrome", async () => {
    const result = await browserProvider.doctor({
      engine: "obscura",
      executable: "/nonexistent/jlc-test-obscura",
    });
  ...
  it("prefers jlc-cli variables while retaining legacy browser paths and settings", () => {
    ...
    const preferred = chromeLaunchArguments(
      "/profile",
      9222,
      {
    ...
    expect(preferred).toContain("--headless=new");
    expect(preferred).not.toContain("--no-sandbox");
  });
  it("keeps sandboxing enabled unless the Linux fixture override is explicit", () => {
    expect(
      chromeLaunchArguments("/private/profile", 9222, { CI: "true" }, "linux"),
  ...
  it("bounds startup diagnostics and removes URL and header credentials", () => {
    const log =
      'No usable sandbox!\nurl=https://alice:PRIVATE_PASSWORD@example.com/start?token=PRIVATE_QUERY#PRIVATE_FRAGMENT\nAuthorization: Bearer PRIVATE_BEARER\nCookie: session=PRIVATE_COOKIE\npassword=PRIVATE_FIELD token="PRIVATE_TOKEN"\n' +
      "x".repeat(20_000);
    const result = sanitizeStartupStderr(log);
    expect(result).toContain("No usable sandbox!");
    expect(result).toContain("https://example.com/start");
    expect(result).not.toContain("PRIVATE_");
    expect(result.length).toBeLessThanOrEqual(4096);
  });
  it("retains a private diagnostic when the browser executable exits before CDP startup", async () => {
    const dir = await directory();
    await expect(
      browserProvider.connect(
        { engine: "chrome", executable: process.execPath, timeoutMs: 5000 },
        dir,
      ),
    ).rejects.toMatchObject({ code: "BROWSER_START_FAILED" });
    const diagnostic = JSON.parse(
      await readFile(join(dir, "browser-startup.json"), "utf8"),
    );
    expect(diagnostic.exitCode).not.toBeNull();
    expect(diagnostic.stderr).toContain("bad option");
    expect(diagnostic.stderr.length).toBeLessThanOrEqual(4096);
    if (process.platform !== "win32")
      expect((await stat(join(dir, "browser-startup.json"))).mode & 0o777).toBe(
        0o600,
      );
  }, 10_000);

  it.skipIf(process.platform === "win32")(
    "a delayed stale-lock observer cannot remove the replacement owner",
    async () => {
      const dir = await directory();
      const lock = join(dir, "browser-connect.lock");
      const ownerFile = join(lock, "owner.json");
      const oldNonce = "a".repeat(48);
      await mkdir(lock);
      await writeFile(
        ownerFile,
        JSON.stringify({ pid: 2147483647, nonce: oldNonce }),
      );
      const oldRead = deferred();
      const resumeOld = deferred();
      const discoveryStarted = deferred();
      const finishDiscovery = deferred();
      let intercepted = false;
      readControl.intercept = async (path) => {
        if (path === ownerFile && !intercepted) {
          intercepted = true;
          oldRead.resolve();
      ...
      const delayed = browserProvider
        .connect(config, dir)
        .catch((error) => error);
      let replacement: Promise<unknown> | undefined;
      try {
        await oldRead.promise;
        replacement = browserProvider
          .connect(config, dir)
          .catch((error) => error);
        ...
        expect(JSON.parse(await readFile(ownerFile, "utf8"))).toEqual(
          currentOwner,
        );
        await expect(
          browserProvider.connect(config, dir),
...
describe.skipIf(!existsSync(chrome))("real CDP browser lifecycle", () => {
  it("preserves the target across CLI-process exit and rejects concurrent CDP clients", async () => {
    ...
    if (process.platform !== "win32") {
      ...
      await writeFile(
        join(dir, "browser-connect.lock", "owner.json"),
        JSON.stringify({ pid: 2147483647, nonce: "b".repeat(48) }),
      );
    }
    ...
    const result = await new Promise<string>((resolveOutput, reject) => {
      ...
      child.on("exit", (code) =>
        code === 0 ? resolveOutput(output) : reject(new Error(error)),
      );
    });
    const { targetId, endpoint } = JSON.parse(result);
    const c = await browserProvider.connect(
    ...
    try {
      ...
      expect(c.targetId).toBe(targetId);
      expect(await c.page.locator("h1").innerText()).toBe("retained scene");
      await expect(
        chromium.connectOverCDP(endpoint, { timeout: 2000 }),
      ).rejects.toThrow();
  ...
  it("keeps cookies and localStorage private and restores them after owned browser restart", async () => {
    ...
    try {
      const first = await browserProvider.connect(
        { engine: "chrome", executable: chrome },
        dir,
      );
      ...
      const saved = JSON.parse(
        await readFile(join(dir, "browser-storage.json"), "utf8"),
      );
      expect(saved.origins).toContainEqual({
        origin: url,
        localStorage: [{ name: "test-session", value: "PRIVATE_LOCAL_VALUE" }],
      });
      // Simulate missing native writes after an abrupt process exit on any OS.
      // Recovery must use the private snapshot, not happen to pass on POSIX.
      await rm(join(dir, "chrome-profile", "Default", "Local Storage"), {
        recursive: true,
        force: true,
      });
      if (process.platform !== "win32") {
        expect(
          (await stat(join(dir, "browser-storage.json"))).mode & 0o777,
        ).toBe(0o600);
      ...
      try {
        ...
        ).toBe("PRIVATE_LOCAL_VALUE");
        expect(await second.page.evaluate(() => document.cookie)).toContain(
          "PRIVATE_COOKIE_VALUE",
        );
        await second.page.evaluate(() => localStorage.clear());
        ...
        await second.page.setContent(
          '<input type="password" value="PRIVATE_PASSWORD"><pre>PRIVATE_COOKIE_VALUE</pre>',
        );
  ...
  it("rejects website WebSockets and CSRF shutdown while native CDP remains usable", async () => {
    ...
    const { token } = JSON.parse(
      await readFile(join(dir, "browser-runtime.json"), "utf8"),
    );
    for (const origin of [
      ...
      const responseCode = await new Promise<number>((resolveCode, reject) => {
        const socket = new WebSocket(
          `${endpoint.replace("http:", "ws:")}/devtools/browser`,
          { headers: { Origin: origin }, handshakeTimeout: 2000 },
        );
        socket.on("open", () => {
          socket.close();
          reject(new Error("Website WebSocket was accepted."));
        });
        socket.on("error", () => {});
        socket.on("unexpected-response", (_request, response) => {
          response.resume();
          socket.terminate();
          resolveCode(response.statusCode!);
        });
      });
      expect(responseCode).toBe(403);
      expect(
        (await fetch(`${endpoint}/jlc/status`, { headers: { Origin: origin } }))
          .status,
      ).toBe(403);
      expect(
        (
          await fetch(`${endpoint}/jlc/stop`, {
            method: "POST",
            headers: { Origin: origin, "x-jlc-instance": token },
          })
        ).status,
      ).toBe(403);
    }
    const reboundStatus = await new Promise<number>((resolveStatus, reject) => {
      const probe = request(
        `${endpoint}/jlc/status`,
        { headers: { Host: "rebound.example:1234" } },
        (response) => {
          response.resume();
    ...
    expect(
      (
        await fetch(`${endpoint}/jlc/status`, {
          headers: { "sec-fetch-site": "cross-site" },
        })
      ).status,
    ).toBe(403);
    expect(
      (await fetch(`${endpoint}/jlc/stop`, { method: "POST" })).status,
    ).toBe(403);
    expect(
      (
        await fetch(`${endpoint}/jlc/stop`, {
          method: "POST",
          headers: { "x-jlc-instance": "wrong-token" },
        })
      ).status,
    ).toBe(403);
    expect(
      (
        await fetch(`${endpoint}/jlc/stop`, {
          headers: { "x-jlc-instance": token },
        })
      ).status,
    ).toBe(405);
    const native = await browserProvider.connect(
      { engine: "chrome", executable: chrome },
      dir,
      target,
    );
...
const obscura =
  executableCandidates("obscura").find((candidate) => existsSync(candidate)) ??
  "";
describe.skipIf(!existsSync(obscura))("real Obscura CDP compatibility", () => {
  it("retains the exact DOM and target across reconnection and persists login storage on restart", async () => {
    ...
    const server = createServer((_request, response) => {
      ...
      response.end(
        "<!doctype html><html><head><title>Obscura fixture</title></head><body><h1>original</h1></body></html>",
      );
    });
    await new Promise<void>((resolveListen) =>
      server.listen(0, "127.0.0.1", resolveListen),
    ...
    try {
      ...
      await active.page.evaluate(() => {
        document.querySelector("h1")!.textContent = "retained scene";
        localStorage.setItem("session-test", "persisted-value");
        document.cookie = "session-test=persisted-cookie; Path=/";
      });
      ...
      expect(await active.page.locator("h1").innerText()).toBe(
        "retained scene",
      );
      expect(
        await active.page.evaluate(() => localStorage.getItem("session-test")),
      ...
      expect(await active.page.evaluate(() => document.cookie)).toContain(
        "persisted-cookie",
      );
      await active.page.evaluate(() => {
        localStorage.clear();
      ...
      if (previous === undefined)
        delete process.env.OBSCURA_ALLOW_PRIVATE_NETWORK;
      else process.env.OBSCURA_ALLOW_PRIVATE_NETWORK = previous;
      await new Promise<void>((resolveClose) =>
        server.close(() => resolveClose()),
      );
    }
