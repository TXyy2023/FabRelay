...
jlc-cli pcb preview --draft UPLOAD_TASK --json
```
...
```text
jlc-cli pcb options --draft PREVIEW_TASK --json
jlc-cli pcb set --draft OPTIONS_TASK --params @values.json --mode default --json
jlc-cli pcb quote --draft SET_TASK --json
```
...
jlc-cli pcb submit --draft CHECK_TASK --json
```
